...
中断后重跑保持同一个 `--output-root`，已完整生成的 `final/` 与 `neurons/` 特征会自动跳过；只有显式加 `--overwrite` 才会覆盖重算。
...
```text
../deepfake-data/cache/tvl1_flow/final/    # RGB 2048 + Flow 2048；BYOL-A 2048
../deepfake-data/cache/tvl1_flow/neurons/  # top-rho 内部神经元拼接表征
```
